...
## 37. v0.9.0: Material You, свободная F-клавиша и таймеры в фоновой вкладке

### Голая «F9» в манифесте ломает установку расширения

Пользователь попросил повесить переключение захвата на клавишу из F-ряда,
которая ничего не делает в браузере. Прописал `"suggested_key": {"default":
"F9"}` — Chrome отказался ставить расширение целиком:

```
Не удалось загрузить расширение: Invalid value for 'commands[1].default': F9
```

В манифесте Chrome требует модификатор — Ctrl или Alt (исключение только для
медиа-клавиш), и одиночную F-клавишу отклоняет. При этом ВРУЧНУЮ, на
chrome://extensions/shortcuts, одиночная F9 назначается свободно.

Поэтому `suggested_key` убран совсем: команда есть, клавиша не назначена, а в
попапе появилась кнопка, открывающая chrome://extensions/shortcuts и
показывающая текущее сочетание (`chrome.commands.getAll()`).

Важное следствие, которое стоит помнить: **после неудачной загрузки Chrome
оставляет расширение в списке, но ВЫКЛЮЧЕННЫМ.** Одного исправления манифеста
мало — расширение нужно включить тумблером обратно.

Свободные клавиши F-ряда в Chrome: F2, F4, F9. Заняты: F1 (справка), F3 (поиск),
F5 (перезагрузка), F6 (адресная строка), F7 (каретка), F10 (меню), F11
(полный экран), F12 (DevTools); F8 перехватывает DevTools, когда они открыты.

### Панель в стиле Material You

Раньше: тёмная плашка с градиентами, обводкой 1.5 px, тяжёлой тенью и жирным
разряженным шрифтом — на светлом интерфейсе платформы смотрелась заплаткой.

Стало по канонам M3: тональные светлые заливки (`#c6f0d2` захват, `#cfe6f7`
dry run, `#ffdad6` тревога, `#ffdfb0` остановлен), никаких градиентов и обводок,
одна ступень тени, форма-таблетка, шрифт 13/500 с обычным трекингом, надписи в
обычном регистре («Захват», «Dry run», «Выкл»). Наведение — слой состояния
(6 % чёрного поверх подложки), как это делает Material, а не смена цвета.
Подтверждение клика — мягкий засвет вместо подпрыгивания.

### Таймеры в фоновой вкладке

Проверка сиротства (раздел 32) висела на `setInterval(2000)`. В ФОНОВОЙ вкладке
Chrome душит таймеры вплоть до одного срабатывания в минуту — а расширение
перезагружают именно тогда, когда смотрят не на платформу. Захвату это не
вредит: он останавливается синхронной проверкой `armed()`. Но панель до минуты
показывала бы «Выкл» вместо «Остановлен · F5» — ровно тогда, когда должна звать
нажать F5.

Проверка вынесена в `watchTick()` и вызывается ещё и на `visibilitychange` и
`focus`. Проверено: вкладка в фоне → перезагрузка расширения → возврат к вкладке
→ панель янтарная «Остановлен · F5» сразу, `data-fct="stopped"`.

node --check 7/7 чисто, manifest → 0.9.0.

## 38. v0.9.1: «Остановлен · F5» читалось как «остановлен клавишей F5»

Пользователь: «что это значит? причём тут F5, который перезагружает страницу».

F5 в надписи был подсказкой к действию, но выглядел как причина остановки —
надпись называла состояние и лишь намекала на выход из него. Заменено на
«Обновите страницу»: это само действие, а не намёк. Подсказка при наведении
объясняет причину целиком: «Расширение обновилось, и движок в этой вкладке
отключён. Нажмите F5 или кликните здесь — захват заработает снова».

Урок общий: короткое состояние в углу экрана читают без контекста, поэтому
в нём должен стоять глагол действия, а не аббревиатура.

node --check чисто, manifest → 0.9.1.

## 39. v0.9.2: размер панели по кнопкам платформы

Пользователь: сделать панель побольше, чуть меньше кнопки «Limit» на самой
платформе.

Измерил её на живой странице: `button.task-filter.flex-center` — 90×37,
шрифт 12/400, скругление 15 px. Панель была 72×27 — заметно мельче всего
остального в интерфейсе.

Стало: padding 10/16/10/12, шрифт 14/500, точка 9 px → высота 34 px при 37 у
«Limit». Ширина зависит от надписи: «Выкл» — 81 px, «Dry run» — 95 px, то есть
и по ширине панель держится в пределах кнопки платформы (90).

Панель осталась в стиле M3 (тональные заливки, одна ступень тени), просто
крупнее — размер теперь читается как элемент того же интерфейса, а не как
случайный ярлык поверх него.

node --check чисто, manifest → 0.9.2.

## 40. v0.9.3: убрана ручная проверка платформы, своё выпадающее меню

### Проверка платформы удалена

Кнопка «Проверить платформу» и `runDiagnostics()` (раздел 33) убраны по просьбе
пользователя: секция занимала в попапе целый экран ради того, что пассивный
сторож ловит сам и без нажатий.

Что ОСТАЛОСЬ и почему: сторож структурных отказов (`noteStructuralFailure()` —
два «не нашёл элемент» подряд), проверка отсутствия таблицы через 15 с после
загрузки, красный бейдж `!`, баннер в попапе и красная панель на странице.
Это работает само, места в интерфейсе не занимает и закрывает тот же класс
проблем — молчаливую поломку селекторов. Из движка удалены только
`runDiagnostics()`, `check()` и обработчик `fct-diagnose`.

Текст баннера больше не отсылает к удалённой кнопке.

### Своё выпадающее меню вместо <select>

Нативный `<select>` Chrome рисует средствами ОС: белый список системным шрифтом
поверх тёмного попапа — было видно на скриншоте пользователя.

`initDropdown(id, onChange)` в popup.js делает список из готовой разметки
(`.dd` → `.dd-btn` + `ul.dd-list[role=listbox]`), значение хранит в
`root.dataset.value`. Ведёт себя как `<select>`, включая клавиатуру:
Enter/Пробел/стрелки открывают, стрелки водят по пунктам, Enter выбирает,
Esc закрывает и возвращает фокус на кнопку, клик мимо закрывает. Выбранный
пункт помечен галочкой и цветом акцента.

Грабли, уже знакомые по разделу 33: `.dd-list` не задаёт `display`, а на всякий
случай продублировано `.dd-list[hidden]{display:none}` — авторский `display`
перебивает браузерное правило для `[hidden]`.

Проверено в браузере: мышью — открытие, выбор, запись в storage; клавиатурой —
Enter → ArrowUp → Enter меняет значение и возвращает фокус, Esc закрывает без
изменения; ошибок на странице нет.

node --check 7/7 чисто, manifest → 0.9.3.

## 41. v0.9.4: убраны подсказки в скобках

Пользователь: «убери эти глупые подсказки», указывая на «(Chrome не даёт задать
F-клавишу заранее)».

Удалены все `<small>` в скобках у тумблеров и блок `#modeHint` («Захват: клик по
кнопке Start Transaction…»), объяснявший очевидное. Вместе с ними — мёртвые
правила `.row small` и `.mode-hint` и функция `renderModeHint()`.

Два факта из подсказок были не декоративными, а поведенческими, и потеряться не
должны — они переехали в `title` (видно при наведении, места не занимает):
«Захват включён» → сбрасывается при перезагрузке страницы; «Dry Run» → заявки
не берутся, только записи в журнале.

Оставлен абзац `p.hint` в «Приоритетах типов»: он описывает правило точного
совпадения набора чипов, которое ниоткуда не выводится и один раз уже стоило
разбирательств.

node --check чисто, manifest → 0.9.4.

## 42. v1.0.0: подготовка к Chrome Web Store, удалён chrome.debugger

### Удалён `debugger`

Разрешение `debugger` — самое чувствительное из тех, что проверяет Google:
это доступ к отладочному протоколу, то есть фактически к чему угодно на
странице. Заявки с ним отклоняют или тянут неделями. Побочный эффект был виден
и пользователю: при срабатывании Chrome вешал полосу «расширение начало отладку
браузера».

Использовался он в одном месте — «доверенный» ПОВТОРНЫЙ клик по кнопке
подтверждения (клики №2 и №3). Первый клик всегда был обычным, а если
`debugger.attach` не удавался, код и так падал обратно на `dispatchClick()`.
То есть запасной путь запасного пути. После починки надписи на кнопке
(раздел 28) обычного клика хватало во всех прогонах.

Удалено: `trustedClick()` в воркере, обработчик `fct-trusted-click`,
`trustedClickAt()` в content script, флаг `dg.trusted`. Повторный клик теперь
обычный.

### Почищены разрешения

Осталось `storage` + `scripting` и ОДИН домен. Из `host_permissions` убран
`https://api.flowconnect-group.com/*` — он остался от удалённого turbo-режима,
кода за ним нет уже несколько версий.

Проверено на живом профиле: `chrome.permissions.getAll()` →
`{permissions: ["storage","scripting"], origins: ["https://alpha.flowconnect-group.com/*"]}`,
захват в Dry Run работает, ошибок нет.

### Сборка пакета

`build/task-taker-<версия>.zip` — только `manifest.json`, `icons/`, `src/`
(14 файлов, ~45 КБ). Архивировать папку проекта целиком НЕЛЬЗЯ: в корне лежит
`fct-insert-body.json` с настоящими ФИО и номерами прав водителей.

Грабли: `Compress-Archive` в PowerShell 5.1 пишет пути в zip через ОБРАТНЫЙ
слеш (`icons\icon16.png`), что нарушает спецификацию zip и может не пройти
загрузку. Пакет собирается через `System.IO.Compression.ZipFile.Open` +
`CreateEntryFromFile` с явной заменой `\` на `/`.

Тексты листинга, обоснования разрешений и порядок публикации — в
`docs/store-listing.md`, политика конфиденциальности — в `PRIVACY.md`.

manifest → 1.0.0.

## 43. v1.0.1: убрано название платформы из всего, что видно снаружи

Пользователь: «убери отовсюду любые упоминания flow и тд».

Убрано:

- имя расширения и `default_title` → «Task Taker»;
- описание в манифесте — без домена и без названия компании;
- подзаголовок `alpha.flowconnect-group.com` из шапки попапа;
- название платформы из README, PRIVACY.md и всех текстов листинга;
- жёстко зашитый домен в popup.js заменён на `FCT.CONFIG.origin` — адрес теперь
  живёт в одном месте (плюс `matches` в манифесте, который шаблоном не задать).

**Чего убрать нельзя, и это надо понимать.** Домен обязан быть в
`host_permissions` и `content_scripts.matches`, иначе расширение не работает
вовсе. Chrome показывает его каждому пользователю при установке («Чтение и
изменение ваших данных на сайте …») и ревьюеру Google. Так что скрытием это не
является — от посторонних защищает только видимость Unlisted/Private и
приватный репозиторий.

Отдельно отмечено в `docs/store-listing.md`: имя репозитория
`flowconnect-task-taker` тоже содержало название (переименовано в `task-taker`
в v1.0.1), а сам `platform-notes.md` —
самый чувствительный файл в публичном репозитории: селекторы, имена API
платформы, поведение её сервера. Прошлые коммиты названия тоже хранят: правка
файлов историю не переписывает.

Названия порталов `eld88` / `flow` в коде оставлены: это литеральные значения
ячейки таблицы, по которым идёт фильтрация, и подписи чипов в попапе. Убрать их
— сломать фильтр по порталам.

manifest → 1.0.1.

## 44. Публикация: репозиторий открыт и переименован

Перед тем как делать репозиторий публичным, история просканирована целиком
(29 коммитов, все когда-либо добавленные файлы):

- `fct-insert-body.json`, `real-insert-body.json`, `resp-*.json` — файлы с
  реальными ФИО и правами водителей — в коммиты не попадали ни разу;
- токенов и паролей нет: упоминания `Authorization: Bearer` в этих заметках
  описывают механизм, а не содержат значения;
- `src/shared/turbo.js`, удалённый вместе с turbo-режимом, но оставшийся в
  истории, — чистый код без вшитых данных;
- `test/mock.html` — без реальных имён и компаний.

Репозиторий переименован `flowconnect-task-taker` → `task-taker`: имя было
последним местом, где название платформы торчало наружу. GitHub держит редирект
со старого адреса, но ссылка на политику конфиденциальности в
`docs/store-listing.md` обновлена на новую — редирект в форме магазина лучше не
использовать.

Публичным остаётся сам `platform-notes.md`: селекторы, имена API платформы и
описание её авторизации. Секретов там нет, но это подробная карта чужой системы.

## 45. Сборка «Task Taker I»: чёрный список компаний

По просьбе пользователя заведена отдельная сборка в папке `task-taker-i/` —
полная копия расширения плюс одна новая функция.

### Чёрный список компаний

Настройка `blockedCompanies` (массив строк) в конфиге. Проверка —
`blockedCompany(row)` в content script: нормализованный текст ячейки компании
сравнивается с нормализованными записями списка **по вхождению подстроки**.
Оператор вводит «rogue» — ловится «ROGUE CARRIER INC»; точное написание из
таблицы помнить не нужно, регистр и лишние пробелы значения не имеют.

Проверка стоит в `performGrab()` ПЕРЕД веткой Dry Run. Иначе в холостом прогоне
запрещённая компания записывалась бы в журнал как «взяли бы», что прямо
противоречит настройке. Сработавшая запись попадает в журнал:
`skip — компания в чёрном списке: rogue`.

В попапе — секция со списком и полем ввода, по образцу приоритетов, но без
стрелок: порядок здесь не значит ничего. Пустой список показывает строку
«список пуст — берутся все компании»: пустота в этом месте слишком похожа на
поломку.

### Что пришлось развести между сборками

Две сборки могут работать в одной вкладке одновременно, а DOM у страницы один:

- `PANEL_ID` → `__fct_panel_i`, `PANEL_POS_KEY` → `__fct_panel_i_pos`,
  атрибут состояния → `data-fct-i`. Без этого две панели дерутся за один узел.
- Иконки перерисованы в фиолетовый (`icons/` варианта): в панели Chrome две
  одинаковые зелёные молнии не различить, а перепутать сборки — значит не
  понять, какая из них взяла заявку.

Что НЕ разводится намеренно: `C.lockPrefix`. Локи лежат в localStorage
страницы, общем для всех расширений, поэтому одинаковый префикс заставляет
сборки видеть локи друг друга и не брать одну заявку дважды.

Хранилища у расширений раздельные (`chrome.storage` привязан к id), так что
настройки и журналы не пересекаются.

### Чем проверено

- Правило сопоставления — на настоящем `FCT.normalizeType` из кода варианта,
  7 случаев с реальными названиями компаний из таблицы платформы: совпадения по
  части названия, разный регистр, лишние пробелы, отсутствие ложных срабатываний.
- `node --check` по всем файлам сборки.

Чего проверить не удалось: сквозной прогон в браузере. Тестовый профиль
запущен с `--disable-extensions-except`, где указан только путь основной
сборки, поэтому вторая туда не встаёт (`Extensions.loadUnpacked` через CDP
проходит, но расширение остаётся заблокированным флагом). Конфиг
`.playwright-mcp-config.json` обновлён на обе папки — подхватится при следующем
старте MCP-сервера.

### Цена решения

Это форк, а не флаг: 14 файлов продублированы. Любая правка движка теперь
требует переноса в обе папки, и рано или поздно они разойдутся. Если сборка
«I» приживётся, её стоит свести обратно в одну кодовую базу — фича включалась
бы настройкой, а не отдельной копией.

## 46. Task Taker I: стоп-слова в комментарии

Второй фильтр отказа в сборке «I»: заявка не берётся, если в её комментарии
встретилось слово из списка. Колонка проверена на живой странице —
`.mat-column-Comment`, и текст в DOM ПОЛНЫЙ (образец на 137 символов), визуально
обрезается только по ширине ячейки. Значит, искать можно по `innerText`.

### Почему правило другое, чем у компаний

Компании сравниваются по вхождению подстроки — оператор пишет «rogue» и ловит
«ROGUE CARRIER INC». Для комментариев так нельзя: это свободный текст, и
подстрока «pu» сработала бы внутри «pick up», а «ne» — внутри «need», «new» и
«note». Поэтому стоп-слова матчатся по ЦЕЛОМУ слову: границы через
`(?<![\p{L}\p{N}])` и `(?![\p{L}\p{N}])` с флагом `u` (нужен Chrome 111+, он у
нас и так в `minimum_chrome_version`). Пробел внутри записи означает разделитель
любой длины, поэтому «pick up» находит и «pick  up» с переносом строки;
спецсимволы регэкспа экранируются, так что «12:57:43» ищется буквально.

Выключатель `wordFilterEnabled` гасит фильтр целиком, не стирая список —
слайдер «Фильтр включён» в этой же секции попапа.

### Правила переехали в engine-core

`findBlockedWord()` и `findBlockedCompany()` вынесены в `engine-core.js`: это
чистая логика без DOM, и в content script остались только две строчки, достающие
текст ячейки. Сделано ради тестов — `test/filters.test.js` гоняет НАСТОЯЩИЙ код,
а не его копию. Копия в тесте однажды уже расходилась бы с кодом незаметно.

21 проверка на реальных комментариях и названиях с платформы, включая
отрицательные: «ne» не ловится внутри need/new/note, «pu» не ловится внутри
«pick up», но ловится отдельным токеном в «PU in 8th at 1pm».

### Грабли инструмента, стоившие двух багов

Bash-heredoc и `node -e` в этой среде съедают одиночный обратный слеш. Из-за
этого в прошлой правке в `addCompany()` регулярка `/\s+/g` превратилась в
`/s+/g` — она вырезала бы из названий компаний букву «s» — а вызов
`renderBlocked()` вставился после КАЖДОГО `renderPriorities()`, а не только в
`init()`. Оба дефекта поймал построчный diff варианта с основной сборкой.
Вывод: правки с регулярками и отступами делать через Edit/Write, а не через
скрипт в командной строке.
